(function(imageproc) {
    ...
    imageproc.comicColor = function(inputData, outputData, saturation) {
        ...
        /*
         * DONE: You need to complete the comic colour function so that
	    ...
	    s = Math.max(Math.min(s * saturation, 1), 0)

	    let rgb = imageproc.fromHSVToRGB(h, s, v)
	    r = rgb.r
	    g = rgb.g
	    b = rgb.b
	    ...
	    let distances = palette.map((arr) => {
		let new_arr = Array.from(arr)
		new_arr[0] -= r
		new_arr[1] -= g
		new_arr[2] -= b
		return new_arr
	    }).map((arr) => Math.hypot(arr[0], arr[1], arr[2]))

	    let argmin = [...distances.keys()].reduce((accum, cur) => (distances[accum] <= distances[cur]) ? accum : cur)
